...
if [[ -z ${FLAGS+x} ]]
  then
    ...
    export FLAGS=" $KIOSK --no-sandbox --disable-dev-shm-usage --ignore-gpu-blacklist --enable-gpu-rasterization --force-gpu-rasterization --autoplay-policy=no-user-gesture-required --user-data-dir=/usr/src/app/settings --enable-features=WebRTC-H264WithOpenH264FFmpeg"
fi
...
echo "xset s xset s noblank" >> /home/chromium/xstart.sh
echo "chromium-browser $CHROME_LAUNCH_URL $KIOSK --disable-dev-shm-usage --user-data-dir=/usr/src/app/settings --window-position=0,0 --no-sandbox --window-position=0,0  --window-size=$WINDOW_SIZE" >> /home/chromium/xstart.sh
